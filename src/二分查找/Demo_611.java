...
//        int[] nums = {24, 3, 82, 22, 35, 84, 19};
    ...
    /**
     * 二分查找<br>
     * 执行用时：258 ms, 在所有 Java 提交中击败了30.36%的用户<br>
     * 内存消耗：41 MB, 在所有 Java 提交中击败了14.78%的用户
     */
    public static int triangleNumber_1(int[] nums) {
        int length = nums.length;
        //两边之和大于第三边，先找出两个元素，然后区间[b, a + b]就是组成三角形的个数
        Arrays.sort(nums);

        int res = 0;
        for (int a = 0; a < length - 2; a++) {//留给后面两个元素
            if (nums[a] == 0)
                continue;
            for (int b = a + 1; b < length - 1; b++) {//留给后面一个元素
                int maxVal = nums[a] + nums[b];

                int left = b + 1, right = length;//如果搜寻到最后一位说明该区间[b + 1, length - 1]都可以组成元素
                while (left < right) {
                    int mid = left + (right - left) / 2;

                    if (nums[mid] > maxVal) {
                        right = mid;
                    } else if (nums[mid] == maxVal) {//查找第一个大于等于两边之和的数返回时还需 -1，不包括该数
                        right = mid;
    ...
    /**
     * 双指针<br>
     * 执行用时：27 ms, 在所有 Java 提交中击败了99.16%的用户<br>
     * 内存消耗：41 MB, 在所有 Java 提交中击败了15.39%的用户
     */
    private static int triangleNumber_2(int[] nums) {
        int length = nums.length;
        Arrays.sort(nums);

        int res = 0;
        for (int a = 0; a < length; a++) {//确定了两条边(另一条边的取值范围就不用每一次都二分搜索了)

            for (int right = a - 1, left = 0; right > left; right--) {

                while (left < right && nums[right] + nums[left] <= nums[a])//使用left < right(退出循环的是left == right)
                    left++;
...
//    static int res = 0;
     ...
     * 回溯(超时)
     */
    private static void backTrack(int startIndex, int[] nums) {
        if (path.size() == 3) {
            if (isValid()) {
//                res++;
        ...
        for (int i = startIndex; i < nums.length; i++) {
            if (nums[i] == 0 || (nums.length - 1 - i < (3 - 1) - path.size()))
                continue;

            path.add(nums[i]);

            backTrack(i + 1, nums);//同层去重
    ...
    private static boolean isValid() {
        int a = path.get(0), b = path.get(1), c = path.get(2);//获取三条边
